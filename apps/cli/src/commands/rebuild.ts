...
`)
  .action(async (projectArg: string | undefined, opts: { quiet?: boolean }) => {
    ...
    if (projectArg) {
      ...
      if (result.success) {
        console.log(chalk.green('✓ 重新编译成功') + chalk.dim(` (${formatDuration(result.durationMs)})`));
        process.exit(0);
      } else {
        if (opts.quiet) {
    ...
    for (const item of selected) {
      console.log(chalk.cyan(`重新编译 ${item.name}...`));
      const result = await runner.rebuildOne(item, { extraArgs });
      if (result.success) {
        console.log(chalk.green('✓ 重新编译成功') + chalk.dim(` (${formatDuration(result.durationMs)})`));
      } else {
        printErrorSummary(result);
