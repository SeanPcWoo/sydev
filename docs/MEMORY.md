...
## 项目概览
- **名称**: sydev — SylixOS 开发环境快速部署工具
- **版本**: 0.4.0
- **主要功能**: workspace/project/device 初始化，工程编译/清理/重新编译，配置模板管理
- **README**: 位于 `/home/sean/project/sydev/README.md`
...
### 8. sydev init — 全流程初始化
- `init --config FILE` — 从 JSON 配置文件一键初始化整个环境
...
基于完整的命令文档，可以开发以下 SKILLS：
1. **sydev-quick-build** — 快速编译单个或多个项目，自动检测并行参数
2. **sydev-config-gen** — 快速生成配置 JSON，支持模板
3. **sydev-env-deploy** — 一键部署完整环境（workspace + projects + devices）
4. **sydev-build-monitor** — 监控编译进度，自动重试失败项目
5. **sydev-template-manager** — 增强模板管理，支持版本控制、共享
